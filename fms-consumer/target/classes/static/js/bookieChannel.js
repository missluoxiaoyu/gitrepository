...
app.controller("chanelctrl",function($scope,$http,cfpLoadingBar,$uibModal,){
	var _get = function(year,month,day){
		$http.get(urlip +'account/bookieAgent/queryAll?year='+year+'&month='+month+'&day='+day)
	...
	$scope.saveOrdrawDetail=function(agentId){
		var detail = function(){  
	        $http.get(urlip +'account/bookieAgent/queryPreStoreDetail?agentCode='+agentId)
	        .success(function(data){
//	        	console.log(data);
	...
	$scope.prestoreAlarm=function(data){
		...
		html += "<label class='youqi'>兑奖率：<input class='confirm_input' id='agentRatecop' value="+ data.agentRate +" readonly onfocus='this.blur()'></label>";	
	...
	$scope.updateAgentName=function(data){	
		...
		html += "<label class='youqi'>兑奖率：<input class='confirm_input' id='agentRateid' value="+ data.agentRate +" readonly onfocus='this.blur()'></label>";	
	...
	$scope.prestore=function(data){	
		...
		html += "<label class='youqi'>兑奖率：<input class='confirm_input' id='agentRateyu' value="+ data.agentRate +" readonly onfocus='this.blur()'></label>";	
		layer.confirm(html, {
			btn: ['取消', '确定']
		}, function() {
			layer.msg('取消了', {
...
//导出功能
	$scope.export=function(){
		...
		if(time =="" || time ==null){
			...
			window.open(urlip +'account/bookieAgent/channelStatisticsExcel?year='+year+"&month="+month+"&day="+day);
  ...
  /*  $scope.items = items1;
    ...
    $scope.ok = function () {
    	...
    	if(str==null || str =="" || end==null || end ==""){
    		...
    		window.open(urlip +'account/bookieAgent/detailExcel?startDate='+str+"&endDate="+end+"&agentId="+agentidd);
