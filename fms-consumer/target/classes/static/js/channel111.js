...
app.controller("chanelctrl",function($scope,$http,cfpLoadingBar,update,add,sell,allsell){
	var _get = function(year,month,day,pid){
		$http.get(urlip +'account//channelStatistics/queryAll?year='+year+'&month='+month+'&day='+day+'&pid='+pid)
		.success(function(data){
			if(data){
...
//	_get();     //初始化
	...
	$scope.updateAgentName=function(agentName,agentCode,agentId,url,password,prestore,prestoreAlarm,upnew){	
		var html = "<label class='youqi'>渠道名称：<input class='confirm_input' id='agentNameid' value="+ agentName +" readonly onfocus='this.blur()'></label>";
	...
	$scope.prestore=function(agentName,agentCode,agentId,url,password,prestore,prestoreAlarm,upnew){	
		...
		html += "<label class='youqi'>预存额：<input class='confirm_input noline' id='prestoreyu' placeholder="+ prestore +" value="+ prestore +" readonly onfocus='this.blur()'></label>";
		layer.confirm(html, {
			btn: ['取消', '确定']
		}, function() {
			layer.msg('取消了', {
...
//	        	console.log(data);
	...
	$scope.recycleDetail=function(agentId){

		var sel=document.getElementById("state");
		var pid = sel.options[sel.selectedIndex].value;
		
		window.open (urlip +'account/channelStatistics/date?agentId='+agentId+'&pid='+pid ,'detail','height=400,width=500,top=200,left=500,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no') 
...
//			var execl = excel.queryallup(year,month,day);
			window.open(urlip +'account/channelStatistics/channelStatisticsExcel?year='+year+"&month="+month+"&day="+day+"&pid="+pid);
