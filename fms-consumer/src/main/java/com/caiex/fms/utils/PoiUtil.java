...
public class PoiUtil {
	...
	private static void insertCopyCell(HSSFSheet sheet, int iRowStart,
		...
		for (int iLoop = 0; iLoop < iDataCount; iLoop++) {
			...
			for (int iLoop1 = 0; iLoop1 < iRowCount; iLoop1++) {
				...
				oldCell = oldRow.getCell(iColStart);
				
				newCell = newRow.createCell(iColStart);
	...
	public static HSSFWorkbook getListToExcel(String strTemplate, int iRowStart, int iColStart,
			List list, String strKeyArray[], Class<?> cls) {
		HSSFWorkbook wb = null;
		...
		try {
			...
			if (wb != null) {
				if (list != null) {
					...
					if(sheet.getLastRowNum() > iRowStart){
						if (list.size() > 0) {
	...
	public static HSSFWorkbook getTListToExcel(String strTemplate, int iRowStart, int iColStart,
			List list, String strKeyArray[], Class<?> cls) {
		HSSFWorkbook wb = null;
		HSSFSheet sheet = null;
	...
	public static HSSFWorkbook getListToExcelIn(InputStream in, int iRowStart, int iColStart,
			List list, String strKeyArray[], Class<?> cls) {
		HSSFWorkbook wb = null;
		HSSFSheet sheet = null;
		...
		try {
			// 从文件路径中读取模板文件
			if(null == in){	//当路径为空时，创建新的文件
				wb = new HSSFWorkbook();
				sheet = wb.createSheet();
			} else {
				wb = inputStreamTemplateFile(in);
			}
			if (wb != null) {
				if (list != null) {
					// 获取第一个sheet的数据
	...
	private static HSSFWorkbook inputStreamTemplateFile(InputStream in) {
		//File templateFile = null;
		//FileInputStream inputStream = null;
		POIFSFileSystem templetIn = null;
		HSSFWorkbook wb = null;
		try {
			...
			if (in!=null) {
				...
				templetIn = new POIFSFileSystem(in);
				wb = new HSSFWorkbook(templetIn);
	...
	public static HSSFWorkbook getDeleteListToExcel(String strTemplate, int iRowStart, int iColStart,
			List list, String strKeyArray[], Class<?> cls) {
		HSSFWorkbook wb = null;
		HSSFSheet sheet = null;
			...
			if (wb != null) {
				if (list != null) {
					...
					for (int iRow = iRowStart; iRow < list.size() + iRowStart; iRow++) {
						...
						if(list.size()<54){
							//sheet.shiftRows(1,4,-1);//删除第1行到第4行，使下方单元格上移
							sheet.shiftRows(49, 70, -1);
						}
						if(list.size()<69){
							//sheet.shiftRows(1,4,-1);//删除第1行到第4行，使下方单元格上移
							sheet.shiftRows(60, 70, -1);
						}
					}
	 ...
	 * @param obj
	 */
	private static void writeCell(HSSFSheet sheet, int iRow, int iCol,
	...
	public static HSSFWorkbook getNullToExcel(String strTemplate) {
		HSSFWorkbook wb = null;
		HSSFSheet sheet = null;
	...
	public static void returnExcel(HttpServletResponse response, HSSFWorkbook workbook, String fileName) {
		...
		//IOUtils.write(workbook.getBytes()
			workbook.write(ouputStream);
			ouputStream.flush();
			ouputStream.close();
		} catch (UnsupportedEncodingException e) {
			System.out.println("在对文件名进行转码时,出现问题,请您输入正确的Encode字符串!");
			e.printStackTrace();
		} catch (IOException e) {
			System.out.println("准备将生成的Excel输出到response时出现问题,原因有可能是response已经关闭或其他问题!");
			e.printStackTrace();
		}
	}
}
